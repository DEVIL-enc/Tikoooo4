...
# ============================
# خطط المعالجة (FAST / SMOOTH+ / ULTRA)
# ============================
FFMPEG_PLANS = {
...
async def process_video(request: Request, file: UploadFile = File(...), plan: str = Form("fast")):
    ...
    try:
        ...
        if plan not in FFMPEG_PLANS:
            plan = "fast"
        cmd = [str(x).format(input=tmp_in_path, output=tmp_out_path) for x in FFMPEG_PLANS[plan]]
        ...
        return JSONResponse(
            status_code=400,
            content={"status": "failed", "message": "فشلت المعالجة", "next_plan": next_plan, "stderr": (e.stderr or "")[:500]}
        )
